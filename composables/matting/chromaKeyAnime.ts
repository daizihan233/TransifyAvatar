...
export interface ChromaAnimeOptions {
  ...
  // 🔧 调试开关：允许临时关闭某些处理步骤
  // enableEdgeDetection?: boolean       // 是否启用边缘检测（默认true）
  // enableFloodFill?: boolean           // 是否启用洪水填充（默认true）
  // enableComplexStructure?: boolean    // 是否启用复杂结构保护（默认true）
  // enableSmallRegionRemoval?: boolean  // 是否启用小区域移除（默认true）
  // enableEdgeErosion?: boolean         // 是否启用边缘腐蚀（默认true）
}

/**
 * 动漫图 Chroma Key 抠图（v3 - 保守背景 + 激进边缘）
 */
export async function removeBackgroundChromaAnime(
  ...
  const minKeepArea = Math.max(0, Math.round(options.minKeepArea ?? 100))
  const useHsv = options.useHsv ?? true
  const autoSample = options.autoSample ?? false
  const edgeThreshold = Math.max(0, Math.min(200, Math.round(options.edgeThreshold ?? 30)))
  const erosionTolerance = Math.max(0, Math.min(3, options.erosionTolerance ?? 1.5))
  const floodFillStrength = Math.max(-1, Math.min(1, options.floodFillStrength ?? 0.5))
  const complexStructureThreshold = Math.max(0, Math.min(10, options.complexStructureThreshold ?? 3))
  const edgePrecision = Math.max(1, Math.min(5, Math.round(options.edgePrecision ?? 2)))
  const minEnclosedArea = Math.max(0, Math.min(9999, Math.round(options.minEnclosedArea ?? 100)))

  // 🎯 读取边缘复杂度分析参数
  const complexityWeights = {
    angleChange: Math.max(0, Math.min(100, options.complexityWeights?.angleChange ?? 35)),
    stdDev: Math.max(0, Math.min(100, options.complexityWeights?.stdDev ?? 35)),
    perimeterArea: Math.max(0, Math.min(100, options.complexityWeights?.perimeterArea ?? 20)),
    circularity: Math.max(0, Math.min(100, options.complexityWeights?.circularity ?? 10))
  }
  const maxEnclosedAreaForComplexity = Math.max(0, Math.min(99999, options.maxEnclosedAreaForComplexity ?? 9999))
  const perimeterAreaNormalizer = Math.max(1, Math.min(100, options.perimeterAreaNormalizer ?? 10))
  ...
  const distArr = conservativeMarkBackground(data, bgMask, w, h, n, { r: tr, g: tg, b: tb, h: th, s: ts, v: tv }, useHsv, tol)

  // Step 2: 边缘检测（edgeThreshold = 0 时禁用）
  let edgeMask = new Uint8Array(n)
  if (edgeThreshold > 0) {
    onProgress?.('edge detection', 70)
    edgeMask = detectSharpEdges(data, w, h, n, edgeThreshold)
  ...
  if (floodFillStrength >= 0) {
    conservativeFloodFill(bgMask, edgeMask, distArr, w, h, { n, tolThreshold: useHsv ? tol / 255 : tol, floodFillStrength })
  ...
  if (complexStructureThreshold > 0 && minEnclosedArea > 0) {
    onProgress?.('complex structure detection', 72)
    protectComplexStructures(bgMask, w, h, n, complexStructureThreshold, minEnclosedArea, edgeMask, distArr, useHsv ? tol / 255 : tol, complexityWeights, maxEnclosedAreaForComplexity, perimeterAreaNormalizer)
  ...
  onProgress?.('postprocess', 75)

  // Step 4: 移除小噪点（minKeepArea = 0 时禁用）
  if (minKeepArea > 0) {
    removeSmallForegroundRegions(bgMask, w, h, n, minKeepArea)
    console.log(`✅ [Step 4] 小区域移除已启用 (最小面积: ${minKeepArea})`)
  } else {
    console.log('⚠️ [Step 4] 小区域移除已禁用 (最小面积=0)')
  }

  onProgress?.('edge processing', 80)

  // Step 5: 精确边缘定位 + 激进腐蚀（erosionTolerance = 0 时禁用）
  if (erosionTolerance > 0) {
    preciseEdgeErosion(bgMask, data, w, h, n, { r: tr, g: tg, b: tb, h: th, s: ts, v: tv }, useHsv, tol, erosionTolerance, edgePrecision)
    console.log(`✅ [Step 5] 边缘腐蚀已启用 (强度: ${erosionTolerance.toFixed(1)}, 精度: ${edgePrecision})`)
  } else {
 ...
 */
function conservativeMarkBackground(
  data: Uint8ClampedArray,
  bgMask: Uint8Array,
  w: number,
  h: number,
 ...
 * 保守洪水填充（严格限制，绝不穿透前景）
 */
function conservativeFloodFill(
  bgMask: Uint8Array,
  edgeMask: Uint8Array,
  ...
  while (queue.length > 0) {
    ...
    const tryExpand = (nx: number, ny: number) => {
      ...
      // 1. 必须已标记为背景
      if (!bgMask[np]) {
        // 如果未标记为背景，检查是否在扩展阈值内且不是边缘
        if (npDist > expandThreshold || isEdge) {
          return
        }
      }

      // 2. 遇到边缘，只有在颜色极度相似时才扩展
  ...
  // ✅ 不再清理未访问的背景像素，保留它们供复杂结构检测使用
  // 这些未访问的背景区域可能是：
  // 1. 前景内部的意外封闭结构（需要移除）
  // 2. 真正的镂空、粒子效果（需要保留）
  // 后续的 protectComplexStructures 函数会判断并处理它们
...
function removeSmallForegroundRegions(
  bgMask: Uint8Array,
  w: number,
  h: number,
  n: number,
...
/**
 * 识别并保护复杂结构（镂空、粒子效果等）
 * 通过量化边缘平滑度来区分真正的复杂结构和意外的封闭结构
 * - 复杂结构（镂空、粒子）：边缘不规则、曲折、角度变化大
 * - 意外封闭结构：边缘平滑、规则、接近圆形或椭圆
 */
function protectComplexStructures(
  bgMask: Uint8Array,
  w: number,
  h: number,
  n: number,
  ...
  console.log(`\n========== [封闭区域检测] 开始 ==========`)
  console.log(`参数: minEnclosedArea=${minEnclosedArea}, complexStructureThreshold=${threshold}`)
  console.log(`高级参数: maxEnclosedAreaForComplexity=${maxEnclosedAreaForComplexity} ${maxEnclosedAreaForComplexity === 0 ? '(不限制)' : ''}`)
  console.log(`复杂度权重: angleChange=${complexityWeights.angleChange}, stdDev=${complexityWeights.stdDev}, perimeterArea=${complexityWeights.perimeterArea}, circularity=${complexityWeights.circularity}`)
  console.log(`周长面积归一化系数: ${perimeterAreaNormalizer}`)
  ...
  for (let p = 0; p < n; p++) {
    ...
    totalEnclosedCount++
    console.log(`\n--- 发现封闭区域 #${totalEnclosedCount} ---`)
    console.log(`  位置: (${component[0] % w}, ${Math.floor(component[0] / w)})`)
    console.log(`  面积: ${component.length} 像素`)
    console.log(`  判断: ${component.length} ${component.length < minEnclosedArea ? '<' : '>='} ${minEnclosedArea}`)
    ...
    if (component.length < minEnclosedArea) {
      ...
      console.log(`  ✅ 动作: 小封闭区域，认为是误判，恢复为前景 (恢复 ${component.length} 像素)`)
      for (const idx of component) {
    ...
    if (component.length > maxEnclosedAreaForComplexity) {
      tooLargeRegionCount++
      console.log(`  ⚠️ 动作: 封闭区域过大（${component.length} 像素 > ${maxEnclosedAreaForComplexity}），跳过复杂度分析`)
      for (const idx of component) {
        bgMask[idx] = 1  // 标记为背景
      }
      continue
    ...
    largeRegionCount++
    console.log(`  → 动作: 大封闭区域，继续分析边缘复杂度`)
    ...
    if (edgePixels.length < 10) {
      console.log(`  ⚠️ 边缘像素太少 (${edgePixels.length} < 10)，跳过分析`)
      continue
    }
    ...
    console.log(`  边缘分析: 周长=${edgePixels.length}, 平滑度=${smoothness.toFixed(2)}, 复杂度=${complexity.toFixed(2)}`)
  ...
  console.log(`\n--- 第一遍扫描完成 ---`)
  console.log(`总封闭区域数: ${totalEnclosedCount}`)
  console.log(`小区域(已恢复): ${smallRegionCount} 个，共 ${restoredPixelCount} 像素`)
  console.log(`大区域(待分析): ${largeRegionCount} 个`)
  console.log(`待边缘复杂度分析: ${enclosedRegions.length} 个`)
  ...
  for (let i = 0; i < enclosedRegions.length; i++) {
    ...
    console.log(`\n区域 #${i + 1}: 面积=${region.area}像素, 周长=${region.perimeter}`)
    console.log(`  平滑度: ${region.edgeSmoothness.toFixed(2)}`)
    console.log(`  复杂度: ${region.edgeComplexity.toFixed(2)} ${isComplexStructure ? '>=' : '<'} ${threshold}`)

    if (isComplexStructure) {
      // 复杂结构（高边缘复杂度）：恢复为前景
      complexStructureCount++
      complexStructurePixels += region.area
      console.log(`  ✅ 结论: 复杂结构（边缘不规则），恢复为前景 (${region.area}像素)`)
      ...
      console.log(`  ❌ 结论: 意外封闭结构（边缘平滑），保持透明 (${region.area}像素)`)
      for (const idx of region.component) {
        bgMask[idx] = 1
      }
    }
  ...
  console.log(`\n--- 第二遍分析完成 ---`)
  console.log(`复杂结构(已恢复): ${complexStructureCount} 个，共 ${complexStructurePixels} 像素`)
  console.log(`平滑结构(保持透明): ${smoothStructureCount} 个，共 ${smoothStructurePixels} 像素`)

  // ✅ 修复：第三遍 - 对恢复为前景的区域，重新执行带颜色容差检查的洪水填充
  if (restoredRegions.length > 0) {
    console.log(`\n--- 第三遍：后处理洪水填充 ---`)
    console.log(`需要处理的恢复区域: ${restoredRegions.length} 个`)
    ...
    console.log(`找到种子点: ${newSeeds.length} 个`)
    ...
    console.log(`后处理洪水填充完成`)
  }

  const totalRestoredPixels = restoredPixelCount + complexStructurePixels
  console.log(`\n========== [封闭区域检测] 完成 ==========`)
  console.log(`📊 总结:`)
  console.log(`  - 总封闭区域: ${totalEnclosedCount} 个`)
  console.log(`  - 小区域直接恢复: ${smallRegionCount} 个 (${restoredPixelCount}像素)`)
  console.log(`  - 复杂结构恢复: ${complexStructureCount} 个 (${complexStructurePixels}像素)`)
  console.log(`  - 平滑结构保持透明: ${smoothStructureCount} 个 (${smoothStructurePixels}像素)`)
  console.log(`  - 总共恢复像素: ${totalRestoredPixels} 像素`)
  console.log(`==========================================\n`)
}
...
function preciseEdgeErosion(
  ...
  // 最大允许距离：edgePrecision 的 70%（防止跨越太远识别错误边缘）
  const maxAllowedDistance = edgePrecision * 0.7

  for (let round = 0; round < rounds; round++) {
    const erodeList: number[] = []
    // 逐轮递增而不是递减，越来越激进
    const currentThresh = baseThresh * (1 + round * 0.15)
    ...
    for (let p = 0; p < n; p++) {
      ...
      // 第三步：在 edgePrecision 范围内，找色差最大的相邻点
      ...
      // 第四步：从色差最大的开始，找到第一个距离合理的候选点作为真实边缘
